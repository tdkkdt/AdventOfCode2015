...
let ``do`` =
    ...
    let gates = System.IO.File.ReadAllLines "..\..\..\Inputs\Day7.txt" |> Array.fold (fun acc l -> parseLine l acc) Map.empty
    let aResult = gates |> calculate "a"
    let newGates = gates.Add ("b", Num aResult)
    let result = newGates |> calculate "a" |> string
    System.IO.File.WriteAllText ("..\..\..\Outputs\Day7_2.txt", result)
